...
namespace arcane {
	...
	void Scene3D::init() {
		...
		tunnel->getMeshes()[0].getMaterial().setAlbedoMap(editor::TextureLoader::load2DTexture(std::string("res/textures/bricks2.jpg"), &srgbTextureSettings));
		tunnel->getMeshes()[0].getMaterial().setNormalMap(editor::TextureLoader::load2DTexture(std::string("res/textures/bricks2_normal.jpg")));
		tunnel->getMeshes()[0].getMaterial().setRoughnessMap(editor::TextureLoader::getWhiteTexture());
