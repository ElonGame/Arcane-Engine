...
namespace arcane {
	...
	void Mesh::LoadData(bool interleaved) {
		// Check for possible mesh initialization errors
		{
			...
			if (vertexCount == 0)
				ARCANE_ERROR("Mesh Creation: Mesh doesn't contain any vertices");

			if (m_UVs.size() != 0 && m_UVs.size() != vertexCount)
				ARCANE_ERROR("Mesh Creation: Mesh UV count doesn't match the vertex count");
			if (m_Normals.size() != 0 && m_Normals.size() != vertexCount)
				ARCANE_ERROR("Mesh Creation: Mesh Normal count doesn't match the vertex count");
			if (m_Tangents.size() != 0 && m_Tangents.size() != vertexCount)
				ARCANE_ERROR("Mesh Creation: Mesh Tangent count doesn't match the vertex count");
			if (m_Bitangents.size() != 0 && m_Bitangents.size() != vertexCount)
				ARCANE_ERROR("Mesh Creation: Mesh Bitangent count doesn't match the vertex count");
		}
