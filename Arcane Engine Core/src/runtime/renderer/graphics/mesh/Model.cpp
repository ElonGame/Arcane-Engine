...
namespace arcane {
	...
	void Model::loadModel(const std::string &path) {
		...
		if (!scene || scene->mFlags & AI_SCENE_FLAGS_INCOMPLETE || !scene->mRootNode) {
			ARCANE_ERROR(std::string("model initialization:") + import.GetErrorString());
			return;
		}
	...
	Texture* Model::loadMaterialTexture(aiMaterial *mat, aiTextureType type, bool isSRGB) {
		...
		if (mat->GetTextureCount(type) > 1)
			ARCANE_ERROR("Mesh Loading: Mesh's default material contains more than 1 texture for the same type, which currently isn't supported by the standard shader");
